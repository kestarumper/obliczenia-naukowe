...
for dsize in 1000:1000:50000
    ...
    @testset "Solve LU $(dsize)×$(dsize)" begin
        (A, p, iterations), timeBuildLU, memBuildLU = @timed buildLU(n, ORIGINAL_A, l, true, true)
        x, time, mem = @timed solveLU(n, A, ORIGINAL_b, l, p)

        push!(plot_x, dsize)
        push!(plot_time, time + timeBuildLU)
        push!(plot_mem, mem + memBuildLU)
        push!(plot_iterations, iterations)
...
p_iterations = plot(plot_x, plot_iterations, xlabel="Matrix size", ylabel="Iterations", seriestype=:scatter, title="Solve LU with choice")
...
png(p_iterations, "SolveLuWithChoice_Iterations")
